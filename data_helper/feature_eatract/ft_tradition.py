...
#BOW feature：word_dict
print("get bow feature....")
vectorizer = CountVectorizer(max_df=0.95,min_df=10)#966431->167697
bow = vectorizer.fit_transform(text_list)
# print(vectorizer.get_feature_names())
# print(bow)
# print(bow.toarray())
print("writting bow feature to file")
f=open(os.path.join(dir_name,'ft',"msn_x_y.txt"),'w+',encoding='utf8')
f.write(str(bow.shape[0])+" "+str(bow.shape[1])+"\n")
# bow_=bow.toarray()
print(bow.shape)
# idx=-1
...
for i,item in enumerate(bow):
    ...
    if(i%10000==0):
        ...
        print("iterrows:",i)
    ind=sorted(item.indices)
    if ind.__len__()==0:#if remove word according its appearance frequency,this should be check
        print("wrong data...")
        continue
    ind_ft=item.todense()[:,ind].tolist()[0]
